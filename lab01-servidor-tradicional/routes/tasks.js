...
router.get('/', async (req, res) => {
    try {
        const { completed, priority, page = 1, limit = 10, description, date, title } = req.query;
        ...
        if (rowsCache[cacheKey]) {
            console.log(`[${new Date().toISOString()}] Usando cache para o usuário: ${req.user.username}`);
            res.setHeader('Content-Type', 'application/json');
            return res.send(JSON.stringify({
                success: true,
                data: rowsCache[cacheKey].rows,
                pagination: rowsCache[cacheKey].pagination
            }, null, 2));
        }
        ...
        const countSql = 'SELECT COUNT(*) as total FROM tasks WHERE userId = ?' +
            ...
            (description ? ' AND description LIKE ?' : '') +
            (date ? ' AND date LIKE ?' : '') +
        ...
        if (description) countParams.push(`%${description}%`);
        ...
        rowsCache[cacheKey] = { rows, pagination };

        const keys = Object.keys(rowsCache);
        if (keys.length > 5) {
            delete rowsCache[keys[0]];
        }
